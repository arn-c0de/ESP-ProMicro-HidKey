// Minimal AES-128-ECB implementation for ATmega32U4
// Optimized for code size and memory efficiency
// Based on public domain tiny-AES-c by kokke
...
// Decrypt single block (16 bytes) using ECB mode
...
// S-box and InvS-box lookup tables (stored in PROGMEM to save RAM)
...
static void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key) {
  ...
  for (i = 4; i < 44; ++i) {
    ...
    if (i % 4 == 0) {
      ...
      tempa[0] = tempa[0] ^ getRconValue(i / 4);
    }
